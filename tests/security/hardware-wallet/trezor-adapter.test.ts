...
describe('TrezorAdapter', () => {
  ...
  beforeEach(() => {
    adapter = new TrezorAdapter('bridge:1');
  });
  ...
  describe('Device Information', () => {
    ...
    it('should return connection status', async () => {
      ...
      expect(status.appName).toBe('Trezor Suite');
  ...
  describe('Account Management', () => {
    ...
    it('should generate different keys than Ledger for same derivation path', async () => {
      // This test ensures Trezor and Ledger generate different keys for testing
      const trezorKey = await adapter.getPublicKey("m/44'/501'/0'/0'");
      
      // Create a Ledger adapter for comparison
      const { LedgerAdapter } = await import('../../../src/security/hardware-wallet/ledger-adapter');
      const ledgerAdapter = new LedgerAdapter('/dev/hidraw0');
      await ledgerAdapter.connect();
      const ledgerKey = await ledgerAdapter.getPublicKey("m/44'/501'/0'/0'");
      await ledgerAdapter.disconnect();

      expect(trezorKey.equals(ledgerKey)).toBe(false);
    });
    ...
    it('should throw when not connected', async () => {
      ...
      await expect(adapter.getAccount(0)).rejects.toThrow('Trezor device is not connected');
    });
    ...
    it('should throw when device is locked', async () => {
      ...
      await expect(adapter.getAccount(0)).rejects.toThrow(HardwareWalletException);
      await expect(adapter.getAccount(0)).rejects.toThrow('Trezor device is locked');
    });
  });
  ...
  describe('Transaction Signing', () => {
    ...
    it('should handle user rejection', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('User rejected the transaction on Trezor device');
    });

    it('should sign multiple transactions with individual confirmations', async () => {
    ...
    it('should continue signing after user rejection if it was just a single transaction', async () => {
      let callCount = 0;
      jest.spyOn(adapter as any, 'simulateUserConfirmation').mockImplementation(async () => {
        callCount++;
        if (callCount === 2) {
    ...
    it('should generate different signatures than Ledger', async () => {
      const derivationPath = "m/44'/501'/0'/0'";
      const trezorResult = await adapter.signTransaction(transaction, derivationPath);

      // Create a Ledger adapter for comparison
      const { LedgerAdapter } = await import('../../../src/security/hardware-wallet/ledger-adapter');
      const ledgerAdapter = new LedgerAdapter('/dev/hidraw0');
      await ledgerAdapter.connect();
      const ledgerResult = await ledgerAdapter.signTransaction(transaction, derivationPath);
      await ledgerAdapter.disconnect();

      expect(trezorResult.success).toBe(true);
      expect(ledgerResult.success).toBe(true);
      expect(trezorResult.signature).not.toEqual(ledgerResult.signature);
    });
    ...
    it('should throw when not connected', async () => {
      ...
      await expect(adapter.signTransaction(transaction, derivationPath))
        .rejects.toThrow('Trezor device is not connected');
    });
  });
  ...
  describe('Configuration', () => {
    it('should use custom configuration', () => {
      const customAdapter = new TrezorAdapter('bridge:2', {
        timeout: 60000,
        requireConfirmation: false,
        blindSigning: true,
        debug: true,
        bridgeUrl: 'http://127.0.0.1:21326',
      });
      ...
      expect(customAdapter['config'].bridgeUrl).toBe('http://127.0.0.1:21326');
    });
    ...
    it('should merge with default configuration', () => {
      ...
      expect(customAdapter['config'].bridgeUrl).toBe('http://127.0.0.1:21325'); // Default value
    });
  });

  describe('User Experience Differences from Ledger', () => {
    beforeEach(async () => {
      await adapter.connect();
    });

    it('should have longer confirmation times than Ledger', async () => {
      // Trezor typically takes longer due to touchscreen interaction
      ...
      expect(result.success).toBe(true);
      // Trezor simulation should take at least 3 seconds base time
      expect(elapsedTime).toBeGreaterThanOrEqual(3000);
    });

    it('should have lower user rejection rate than Ledger', async () => {
      // This is a statistical property test - Trezor users tend to be more careful
      // We'll just verify the rejection rate is configured lower
      const mockRejectionRate = 0.03; // 3% vs Ledger's 5%
      
      // Mock Math.random to simulate rejection rate
      const originalRandom = Math.random;
      Math.random = jest.fn(() => mockRejectionRate - 0.01); // Just below threshold
  ...
  describe('Error Handling', () => {
    it('should handle connection errors gracefully', async () => {
      ...
      await expect(adapter.connect()).rejects.toThrow('Failed to connect to Trezor device');
    });
    ...
    it('should handle disconnection errors gracefully', async () => {
      ...
      jest.spyOn(adapter as any, 'simulateDelay').mockImplementation(async () => {
        throw new Error('Bridge disconnection failed');
      });

      await expect(adapter.disconnect()).rejects.toThrow(HardwareWalletException);
      await expect(adapter.disconnect()).rejects.toThrow('Failed to disconnect from Trezor device');
    });
