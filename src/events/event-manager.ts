...
export class EventManager implements EventProcessor {
   ...
   */
  constructor(options: EventManagerOptions = {}, dbManager?: DatabaseManager) {
    ...
    this.emitter.on('error', error => {
      this.logger.error(
        `Error in event emitter: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private wrapHandler<T extends EventName>(
    ...
    return wrappedHandler as {
      (data: EventMap[T]): Promise<void>;
      originalHandler: EventHandler<T>;
    };
  }
  ...
  private async storeEvent<T extends EventName>(event: T, data: EventMap[T]): Promise<void> {
    if (!this.options.storeEvents || !this.db || this.options.excludedEvents.includes(event)) {
      return;
    }
