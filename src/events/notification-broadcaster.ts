...
export class NotificationBroadcaster {
      ...
      this.handlers.set('discord', {
        send: async (notification: NotificationEvent) => {
          try {
            ...
            this.logger.error(`Failed to send Discord notification: ${error instanceof Error ? error.message : String(error)}`);
            return false;
          }
        }
      });
  ...
  private async broadcastNotification(notification: NotificationEvent): Promise<void> {
    ...
    if (channels.length === 0) {
      channels = Array.from(this.handlers.keys()) as ('console' | 'telegram' | 'discord' | 'email')[];
    }
    ...
    const results = await Promise.all(
      channels.map(async (channel) => {
        ...
        try {
          ...
          this.logger.error(`Error sending notification to ${channel}: ${error instanceof Error ? error.message : String(error)}`);
          return false;
        }
      })
    );
    ...
    if (successCount === 0 && channels.length > 0) {
      ...
      this.logger.warning(`Sent notification to ${successCount}/${channels.length} channels: ${notification.title}`);
    }
  }
